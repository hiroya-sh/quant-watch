...
#### 📌 item への詳細追記ルール（必須・Hiroyaさん指定）
item への追記は **基本的に論文の内容を理解したい場合**に行う。追記時は以下の条件を**必ず**満たすこと:

1. **日本語でなるべく詳細に**記述する。
2. **対応する原著部分を逐一併記する**（引用・該当箇所の明示。AIによる誤り/捏造の防止策）。
3. **図は inline 表記**で埋め込む。Obsidian の **ダークモードで見える配色**に注意する（背景透過 or 暗背景前提、線・文字は明色など）。
